...
- **Memoria basada en evidencias (`src/brain/memory.ts`)**: cada análisis de sesión, nota o conversación aporta *evidencias*; el estado lo calcula el código: 1 = observación, 2 = hipótesis (se vigila), ≥3 = regla provisional (se aplica), ≥5 sin contradicciones = consolidada. Lo que cuenta el atleta (nota o chat) suma como mucho una evidencia por aprendizaje y día. Cada evidencia en contra baja un nivel; una evidencia en contra **grave** (lesión, dolor agudo, sobreentrenamiento; solo en lesiones y fatiga) lo descarta hasta que sume 3 evidencias a favor posteriores. Un aprendizaje caduca a los 90 días sin evidencias nuevas y se reactiva con todo su historial si vuelve a pasar. Una misma sesión cuenta una sola vez, y como mucho entran 3 evidencias por evento (primero las contradicciones). Lo que Miguel saca del chat queda pendiente hasta que lo confirmas en *Memoria de Miguel* (sin duplicados, máximo 20, se borran a los 30 días); si ese día ya contó la sesión, no suma dos veces.
- **Motor de readiness (`src/brain/readiness.ts`)**: decide el nivel del día y los límites máximos de la sesión. Si hay fatiga de base (rojo) y además TSB < −30, carga semanal muy alta o estrés ≥ 8, manda descanso obligatorio; lo mismo con ámbar y dos de esas señales. El servidor recalcula siempre el estado con el check-in y la carga (al adaptar la sesión, en el chat y en el plan): el estado que envía la app solo puede endurecerlo, nunca relajarlo. Límites: ámbar = como mucho el 75 % de lo planificado, en verde y sin series; rojo = ≤ 35 min regenerativos o descanso; sin datos de recuperación = suave y sin series. Sin check-in también se evalúa con la carga (TSB, TSS de 7 días). El plan semanal recorta la sesión de hoy a esos límites.
...
- **Carreras preparatorias con fuentes**: al añadir una carrera se busca en Google (búsqueda de Gemini) y solo se guardan los datos que respalda alguna página, con su enlace. Cada cifra tiene que aparecer en un fragmento respaldado por una fuente; cada texto necesita al menos la mitad de sus palabras clave (sin contar genéricas como «terreno») en un mismo fragmento; lo que no, queda *sin verificar*. Un desnivel imposible para la distancia (> 250 m/km) se descarta, y se avisa si distancia y desnivel salen de páginas distintas. Del consejo de Miguel se quitan las frases con cifras de km o metros que no estén verificadas ni se deriven de ellas (diferencias y sumas con la Transvulcania, metros por km); si no queda ninguna, no hay consejo. Lo que escribes tú se marca como dato tuyo. Requiere `GEMINI_API_KEY` aunque uses Experiential.

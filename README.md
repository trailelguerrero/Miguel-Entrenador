...
- `server/rag/`: Supabase (`supabase.ts`), embeddings intercambiables (`embeddings.ts`), biblioteca (`knowledge.ts`), conversaciones (`chatStore.ts`) y troceado (`chunker.ts`).
